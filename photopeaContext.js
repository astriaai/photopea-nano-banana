...
class PhotopeaContext {
  ...
  async initialize() {
    const response = await fetch(`photopea.js`);
  ...
  async pasteImageOnPhotopea(imageURL, bound, layerName = 'image') {
    ...
    return new Promise((resolve, reject) => {
      ...
      const waitTranslate = setInterval(async () => {
        try {
          ...
          const status = await this.invoke('translateIfNewLayerAdded', layerCount, bound, layerName);
          if (status === 'success') {
...
;
function boundWidth(bound) {
  return bound[2] - bound[0];
}
function boundHeight(bound) {
  return bound[3] - bound[1];
}
window.photopeaContext = new PhotopeaContext();
